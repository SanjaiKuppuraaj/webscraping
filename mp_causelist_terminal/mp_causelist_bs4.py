...
def get_valid_session(max_retries=5, use_proxy=False):
    if not use_proxy:
...
def get_csrf_token(session):
    try:
        response = session.get("https://mphc.gov.in/causelist", timeout=90)
...
def robust_request(session, url, headers, retries=5, backoff=5):
    for attempt in range(retries):
        try:
            res = session.post(url, headers=headers, timeout=180)
...
def fetch_mp_causelist(date, city, refresh):
    ...
    for mfb_val, mfb_code in mfb_map.items():
        for sd_val, sd_name in sdbench_map.items():
            try:
                session, port = get_valid_session(use_proxy=use_proxy)
                ...
                with open(filepath, "w", encoding="utf-8") as f:
                    json.dump({"success": True, "message": "Record Found.", "results": merged_data}, f, ensure_ascii=False, indent=2)

                final_summary.extend(merged_data)
                write_log(city_full.capitalize(), bench_full.replace("_bench", "").capitalize(), mfb_full.capitalize(), date, "Completed", current_ip)

                time.sleep(random.randint(15, 22))  # randomized delay
