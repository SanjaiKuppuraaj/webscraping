...
def write_log(date_pdf, update_flag, status, total_cases=0):
    ...
    line = f"[{ts}] date={date_pdf} | update={'true' if update_flag else 'false'} | status={status} | total_cases={total_cases}\n"
...
def parse_case_no(cn):
    ...
    if parts[0] in {'R','F'}:
        return {'case_mode':parts[0],'case_type':parts[1],'case_number':parts[2],
                'case_year':parts[3] if len(parts)>3 else ''}
    else:
        return {'case_mode':'','case_type':parts[0],'case_number':parts[1],
                'case_year':parts[2] if len(parts)>2 else ''}


def download_pdf(listing_date):
    for f in glob.glob(f"{PDF_FOLDER}/*.pdf"): os.remove(f)
    for f in glob.glob(f"{HTML_FOLDER}/*.html"): os.remove(f)

    url = "https://gujarathc-casestatus.nic.in/gujarathc/printBoardNew"
    payload = {
        "coram": "", "coramOrder": "", "sidecode": "", "listflag": "5", "courtcode": "0",
        "courtroom": "undefined-undefined-undefined", "listingdate": listing_date,
        "advocatecodeval": "", "advocatenameval": "", "ccinval": "", "download_token": ""
    }
...
def extract_pdf_to_html(pdf_path):
    output_html_path = os.path.join(HTML_FOLDER, "cause_list.html")
    tagging_keywords = [
        "FOR ADMISSION","FRESH MATTERS","NOTICE & ADJOURNED MATTERS","FOR HEARING",
        "CRITICALLY OLD MATTERS","MR MANAN A SHAH On","MATTERS WHERE",
        "FOR ADMISSION Old Matters - 5 to 10 years","NOTICE & ADJOURNED MATTERS Old Matters - 5 to 10 years"
    ]
    board_mark_keywords = ["ARISING FROM", "CR-I/", "F/CR.MA", "R/CR.A/", "TITLE-", "STATUS-", "POLICE STATION"]
    ...
    with open(output_html_path, "w", encoding="utf-8") as f:
        f.write("""<!DOCTYPE html><html><head><meta charset="UTF-8"><title>PDF Tables</title>
<style>body{font-family:Arial,sans-serif;margin:20px;}table{width:100%;border-collapse:collapse;margin-bottom:30px;border:2px solid #000;}th,td{border:1px solid #000;padding:10px;text-align:left;vertical-align:top;}th{background-color:#f2f2f2;}tr.or{background-color:#e6f7ff;font-weight:bold;}tr.tagging{background-color:#fff8e1;font-style:italic;}h3{margin-top:40px;}</style>
</head><body>""")
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages[1:], start=2):
                ...
                for table_index, table in enumerate(tables, start=1):
                    f.write(f"<h3>Page {page_num} - Table {table_index}</h3>\n<table>\n")
                    grouped_rows = []
...
def parse_html_to_json(html_path, listing_date):
    all_grouped = []
    ...
    for row in rows:
        ...
        if court_match: active_court_no = court_match.group(1)

        judge_lines = [re.sub(r'\s+',' ',l.strip()) for l in row_text.splitlines() if "HONOURABLE" in l.upper() or "HON'BLE" in l.upper()]
        if judge_lines: active_judges = judge_lines[:4]
        ...
        if 'tagging' in row_class:
            ...
            if current_matter_type not in grouped_by_matter_type:
                grouped_by_matter_type[current_matter_type] = {"clist":[],"court_no":active_court_no,"judge_names":active_judges if active_judges else ["NA"]}
            continue
        if 'or' not in row_class: continue
        ...
        for line in lines:
            if re.fullmatch(r'V/?S', line, re.IGNORECASE):
                ...
                current.append(line)
        if found_vs: respondent_lines = current
        else: continue

        petitioner, respondent = clean(petitioner_lines), clean(respondent_lines)
        if not petitioner or not respondent: continue

        main_case = parse_case_no(case_nos[0])
        sol = {'brd_slno':sno_value,'case_no':case_nos[0],'case_mode':main_case['case_mode'],'case_type':main_case['case_type'],
               'case_number':main_case['case_number'],'case_year':main_case['case_year'],'court_name':courts[0] if courts else '',
               'petitioner':[petitioner],'respondent':[respondent],'pet_advocate':tds[3].get_text(strip=True),
               'resp_advocate':tds[4].get_text(strip=True),'connected_cases':[]}

        for cidx in range(1,len(case_nos)):
            parsed = parse_case_no(case_nos[cidx])
            sub_case = {'brd_slno':f"{sno_value}.{cidx}",'case_no':case_nos[cidx],'case_mode':parsed['case_mode'],
                        'case_type':parsed['case_type'],'case_number':parsed['case_number'],'case_year':parsed['case_year'],
    ...
    for matter_type, info in grouped_by_matter_type.items():
        ...
        all_grouped.append({"matter_type":matter_type,"court_no":court_no,"date":date_match or "NA","judge_names":judge_lines_list,"clist":info["clist"]})
...
def process_causelist(date_pdf):
    try:
        ...
        html_path = extract_pdf_to_html(pdf_path)
        result_json = parse_html_to_json(html_path, date_pdf)

        json_path = os.path.join(JSON_FOLDER, date_pdf.replace("/", "-") + ".json")
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(result_json, f, indent=4, ensure_ascii=False)

        total_cases = sum(len(m["clist"]) for m in result_json)
...
def gujarat_causelist():
    ...
    if update_flag:
        threading.Thread(target=process_causelist, args=(date_pdf,), daemon=True).start()
        ...
        if os.path.exists(json_file):
            with open(json_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            total_cases = sum(len(m["clist"]) for m in data)
