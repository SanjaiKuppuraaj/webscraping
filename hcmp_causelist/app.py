...
mp_causelist_blueprint = Blueprint("mp_causelist", __name__)
    ...
    for h2 in soup.find_all('h2'):
        ...
        for tr in h2.find_all_next('tr'):
            ...
            try:
                ...
                sol['cases'] = sol['case_type'] + '-' + sol['case_no'] + '-' + sol['case_year']
...
def log_status(city, mfb, bench, date, status):
    log_folder = cm.BASE_DIR_LOGS + '/' + datetime.now().strftime('%Y-%m-%d') + '/'
    os.makedirs(log_folder, exist_ok=True)
    log_path = os.path.join(log_folder, "mp_causelist.txt")
    log_line = f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | {city}/{mfb}/{bench}/{date} | Status: {status}\n"
    with open(log_path, "a", encoding="utf-8") as log_file:
        log_file.write(log_line)


@mp_causelist_blueprint.route("")
def get_mp_causelist():
    ...
    with sync_playwright() as p:
        ...
        if proxy_settings:
            browser = p.chromium.launch(headless=True, args=["--no-sandbox", "--disable-web-security"], proxy=proxy_settings)
        else:
        ...
        page.goto("https://mphc.gov.in/causelist", wait_until="domcontentloaded", timeout=120000)
        page.select_option('.field-items select#my_city', value=city_val)
            ...
            for sd_val, sd_name in sdbench_map.items():
                ...
                for _ in range(5):
                    try:
                        ...
                        page.select_option('select#aw1', value='0')
                        page.fill("input#code", captcha_text)
                        ...
                        for section in parsed_data:
                            ...
                            section["bench"] = str(sd_name).replace('Bench','').strip()
                            section["court_name"] = city_name

                        filename = f"{city_name.lower()}_{mfb_name.lower().replace(' ', '_')}_{sd_name.lower().replace(' ', '_')}_{date}.json"
                        ...
                        log_status(city_name, mfb_name, sd_name, date, "Completed")
                        success = True
                        break
                    except Exception as e:
                        continue
                if not success:
                    log_status(city_name, mfb_name, sd_name, date, "Error")
        browser.close()
    # return jsonify({"success": True,"message": "Record Found.", "ip_address": ip_address, "results": final_summary})
